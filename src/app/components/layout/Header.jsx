...
const Header = () => {
  ...
  const mobileTopBarItems = useMemo(
    () => [
      { label: 'Campus View', href: 'https://virtualtour.kalingauniversity.ac.in/' },
      ...topBarItems
    ],
    [topBarItems]
  );
  ...
  return (
    <header className="relative z-[10040] w-full overflow-visible">      {/* Top Bar — higher z-index than main nav so mega menus are not hidden behind it */}
      <div suppressHydrationWarning className="text-xs w-full relative z-[10070] overflow-visible">
        <div suppressHydrationWarning className="container mx-auto px-2 pt-3 pb-2 border-b border-gray-200 overflow-visible">
          <div suppressHydrationWarning className="flex justify-between lg:justify-center items-center gap-6 font-medium gap-2 text-[var(--dark-gray)] flex-wrap lg:flex-nowrap overflow-visible w-full">
            ...
            <div suppressHydrationWarning className="relative flex">
              <button
                type="button"
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <MobileMenu navItems={navItems} topBarItems={mobileTopBarItems} onClose={() => setIsMobileMenuOpen(false)} />
      )}
    </header>
  );
};
